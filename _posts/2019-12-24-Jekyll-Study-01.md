...
| `_includes`                                          | `_includes`文件夹内通常存放着一些功能性的页面部件，比如header（页头文件）、footer（页脚文件）等。你可以直接加载这些<b>包含部分<b>（这是一个名词）到你的布局或者文章中以方便重用。<b>比如可以使用include将`_includes `文件夹内的文件直接加载到页面内进行调用。<b> |
