...
<b><font color=DodgerBlue>在本地启动jekyll服务时，可以挂载以下命令项或是上面build的命令项，已达到相应的目的。</font><b>
...
在编译(build)或者生成(serve)运行参数中，你可以指定 Jekyll 的环境和参数。
...
那么，在默认情况下编译你的 Jekyll 网站时，if 条件语句块中的内容不会被执行；除非你在 build 命令中还指定了一个 `production` 环境，像这样：
...
假设您想给站点中的所有页面和文章设置一个默认的布局，那你需要将以下语句添加到你的 `_config.yml` 文件：
